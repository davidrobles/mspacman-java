...
public class PacUCTWithGhostsMinimaxing implements PacAgent, AgentView
{
    private PacUCTNodeMin root;
    ...
    public Direction direction(GameState gameState)
    {
        ...
        root = new PacUCTNodeMin(gameState, null, 0);
    ...
    private void playOneSequence(PacUCTNodeMin root)
    {
        LinkedList<PacUCTNodeMin> list = new LinkedList<PacUCTNodeMin>();
        list.add(root);

        // play for 30 time steps only
        while (list.getLast().getDepth() < MAX_DEPTH) {
            PacUCTNodeMin selectedChild = descendByUCB1(list.getLast());
    ...
    private PacUCTNodeMin descendByUCB1(PacUCTNodeMin parentNode)
    {
        PacUCTNodeMin bestNode = null;
        Double bestValue = Double.MIN_VALUE;

        for (PacUCTNodeMin childNode : parentNode.getNextGameStates())
        {
    ...
    private void updateValue(LinkedList<PacUCTNodeMin> nodes, double value)
    {
    ...
    private Direction getBestMove(PacUCTNodeMin node)
    {
        PacUCTNodeMin bestChild = null;
        double bestVisits = -1;

        // for each next move from the current game state
        for (PacUCTNodeMin nextStateNode : node.getNextGameStates())
        {
    ...
    private void drawNode(Graphics2D dbg2D, PacUCTNodeMin pacUCTNode, int offsetX, int offsetY, int nodeSize)
    {
        // green
        ...
        for (PacUCTNodeMin nextState : pacUCTNode.getNextGameStates())
